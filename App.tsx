    ...
    if (isStarted && currentStep === Step.SUMMARY) {
      const generateReport = async () => {
        ...
        try {
          ...
          setError("שגיאה בייצור הדוח הסופי.");
        } finally {
          setLoadingAi(false);
        }
      };
          ...
          <div className="flex flex-col items-center space-y-10">
            ...
            <div className="space-y-4">
              ...
              <p className="text-xl md:text-2xl text-slate-400 font-medium max-w-2xl mx-auto leading-relaxed">
                מנהלים פסיכולוגיה. מעצבים עתיד. כלי אסטרטגי חכם לבניית תוכניות עבודה מרשימות.
              </p>
            </div>
            ...
            <p className="text-slate-500 text-sm font-medium">פותח עבור מנהלי שירותים פסיכולוגיים בישראל</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-300">
          <div className="glass-card max-w-2xl w-full p-10 md:p-16 rounded-[3rem] border-amber-500/20 shadow-2xl text-center space-y-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-400 italic">זמן לחשוב בזוגות או אישית על הליבה:</p>
            </div>
                 ...
                 <div className="mt-12 p-8 bg-blue-500/5 rounded-3xl border border-blue-500/10 flex flex-col md:flex-row items-start gap-8">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-blue-500/10">
                        ...
                        <div className="text-xs">
                          <span className="text-blue-400/60 font-black uppercase tracking-widest block mb-1">דוגמה</span>
        ...
        <aside className="w-full lg:w-80 shrink-0 space-y-6">
          <div className="glass-card p-8 rounded-[2.5rem] border-amber-500/10">
            ...
            <button 
              ...
              disabled={loadingAi}
              className="w-full py-4 bg-slate-900 border border-slate-800 rounded-xl mb-6 font-bold flex items-center justify-center gap-3 hover:bg-slate-800 transition-colors disabled:opacity-50"
            >
              {loadingAi ? <div className="animate-spin h-5 w-5 border-b-2 border-amber-500 rounded-full" /> : <><Zap className="w-4 h-4 text-amber-500" /> קבל תובנות אסטרטגיות</>}
            </button>
            ...
            <div className="bg-slate-950/40 p-5 rounded-2xl border border-slate-800 min-h-[350px] text-[13px] leading-relaxed text-slate-300">
              {aiSuggestions ? (
                <div className="whitespace-pre-wrap animate-in fade-in duration-500">{aiSuggestions}</div>
              ) : (
                <div className="flex flex-col items-center justify-center h-full opacity-20 text-center space-y-4 py-10">
                  <BrainCircuit className="w-16 h-16" />
                  <p>ה-AI מחכה לנתונים שלך כדי להציע רעיונות לשיפור התוכנית</p>
                </div>
              )}
            </div>
          </div>
...
function renderStepContent(step: Step, data: WorkPlanData, setData: any, finalAiReport: string, activeTab: string, setActiveTab: any) {
  ...
  switch (step) {
    case Step.CONTEXT:
      return (
        <div className="space-y-6">
          <textarea className={inputClass + " h-80"} placeholder="תאר את הרקע, הסביבה העירונית והתחושות הנוכחיות של היחידה..." value={data.selfContext} onChange={(e) => setData({...data, selfContext: e.target.value})} />
        </div>
      );
    case Step.SWOT:
      return (
    ...
    case Step.VISION:
      return <textarea className={inputClass + " h-64 font-serif italic text-4xl text-center pt-20 border-amber-500/10"} placeholder="ניסוח החזון שלכם..." value={data.vision} onChange={(e) => setData({...data, vision: e.target.value})} />;
    case Step.GOALS:
      return (
        <div className="space-y-8">
          <div className="flex gap-4">
            ...
            <button onClick={() => { const el = document.getElementById('goal-in') as HTMLInputElement; if(el.value) { addItem('highLevelGoals', el.value); el.value=''; } }} className="bg-amber-500 text-slate-950 px-12 rounded-2xl font-black">הוסף</button>
          </div>
          <div className="space-y-4">
            {data.highLevelGoals.map((g, i) => (
              <div key={i} className="flex justify-between items-center bg-slate-900/40 p-10 rounded-[2.5rem] border border-slate-800">
      ...
      return (
        <div className="space-y-12">
          {data.highLevelGoals.map((goal, gIdx) => (
            <div key={gIdx} className="p-12 border border-slate-800 rounded-[3.5rem] bg-slate-950/20 relative">
              <div className="absolute -top-5 right-12 bg-slate-900 border border-slate-800 px-6 py-2 rounded-full text-xs text-amber-500 font-black">מטרה: {goal}</div>
              <div className="flex gap-4 mb-10">
                ...
                <button onClick={() => { const el = document.getElementById(`obj-in-${gIdx}`) as HTMLInputElement; if(el.value) { addItem('objectives', { id: Date.now().toString(), goalIndex: gIdx, text: el.value }); el.value=''; } }} className="bg-slate-800 text-white px-10 rounded-2xl font-bold">הוסף יעד</button>
              </div>
              <div className="space-y-4">
                {data.objectives.filter(o => o.goalIndex === gIdx).map(obj => (
                  <div key={obj.id} className="flex justify-between items-center bg-slate-900/60 p-6 rounded-2xl border border-slate-800">
        ...
        <div className="space-y-12">
          {data.objectives.map(obj => (
            <div key={obj.id} className="p-10 border border-slate-800 rounded-[3rem] bg-slate-950/20 shadow-xl">
              <h4 className="font-black text-xl text-white mb-10">יעד: {obj.text}</h4>
                ...
                {data.tasks.filter(t => t.objectiveId === obj.id).map(task => (
                  <div key={task.id} className="flex justify-between items-center bg-slate-950/60 p-6 rounded-2xl border border-slate-800 text-sm shadow-md">
                    <div className="flex gap-8 items-center flex-1">
                      ...
                      <span className="flex items-center gap-2 text-slate-400"><User className="w-4 h-4"/> {task.responsibility || '-'}</span>
                      <span className="flex items-center gap-2 text-slate-500 italic"><Calendar className="w-4 h-4"/> {task.timeline || '-'}</span>
                    </div>
    ...
    case Step.CONSTRAINTS:
      return <textarea className={inputClass + " h-80 border-red-500/10"} placeholder="פרט כאן אילוצים וסיכונים שעלולים לעכב את התוכנית..." value={data.constraints} onChange={(e) => setData({...data, constraints: e.target.value})} />;
    case Step.SUMMARY:
      return (
        <div className="space-y-12">
          <div className="flex justify-between items-end border-b border-slate-800 pb-10">
            <div><h2 className="text-4xl font-black text-white">תוכנית עבודה אסטרטגית שנתית</h2><p className="text-slate-500 italic mt-2">הגרסה המאוחדת של השפ"ח</p></div>
            <div className="flex p-1.5 bg-slate-950 rounded-2xl border border-slate-800 no-print">
              <button onClick={() => setActiveTab('original')} className={`px-8 py-3 rounded-xl text-sm font-black transition-all ${activeTab==='original'?'bg-slate-800 text-white shadow-xl':'text-slate-600'}`}>גרסת טיוטה</button>
              <button onClick={() => setActiveTab('ai')} className={`px-8 py-3 rounded-xl text-sm font-black flex items-center gap-2 transition-all ${activeTab==='ai'?'bg-amber-500 text-slate-950 shadow-xl':'text-slate-600'}`}><Sparkles className="w-4 h-4"/> גרסת המאסטר (AI)</button>
            </div>
          </div>
          <div className="glass-card rounded-[3.5rem] p-10 md:p-20 shadow-2xl relative bg-slate-950/40 border border-slate-800 min-h-[500px]">
            {activeTab === 'original' ? (
              <div className="space-y-16">
                <section className="text-center"><h4 className="text-amber-500 font-black text-[10px] uppercase tracking-[0.4em] mb-8">חזון השירות</h4><p className="text-5xl font-serif italic text-white leading-tight">"{data.vision || 'טרם הוגדר חזון'}"</p></section>
                <section>
                   ...
                   <div className="overflow-x-auto rounded-[3rem] border border-slate-800 shadow-2xl">
                     <table className="w-full text-sm">
                       <thead className="bg-slate-900 text-slate-400"><tr><th className="p-8 text-right">יעד SMART</th><th className="p-8 text-right">משימה</th><th className="p-8 text-right">אחריות</th><th className="p-8 text-right">לו''ז</th></tr></thead>
                       <tbody>
                        {data.objectives.map(obj => {
                          const objTasks = data.tasks.filter(t => t.objectiveId === obj.id);
                          return objTasks.length === 0 ? <tr key={obj.id} className="border-t border-slate-900"><td className="p-8 font-black text-amber-500/70">{obj.text}</td><td colSpan={3} className="p-8 italic opacity-20 text-center">אין משימות</td></tr> :
                          objTasks.map((t, idx) => <tr key={t.id} className="border-t border-slate-900/50">
                            {idx === 0 && <td rowSpan={objTasks.length} className="p-8 font-black text-amber-500 align-top border-l border-slate-900 w-1/4">{obj.text}</td>}
                            <td className="p-8 text-slate-200">{t.description}</td>
                            <td className="p-8 text-slate-400">{t.responsibility}</td>
                            <td className="p-8 text-slate-500 italic">{t.timeline}</td>
                          </tr>);
                        })}
                       </tbody>
                     </table>
          ...
          <div className="flex justify-center pt-10 no-print"><button onClick={() => window.print()} className="flex items-center gap-5 bg-white text-slate-950 hover:bg-amber-500 transition-all px-16 py-6 rounded-[2.5rem] font-black shadow-2xl group active:scale-95"><Save /> הדפס וייצא תוכנית עבודה</button></div>
        </div>
      );
    default: return null;
  }
