...
import { 
  ...
  MessageSquare, Quote, PlayCircle, RefreshCcw, Download
} from 'lucide-react';
    ...
    if (isStarted && currentStep === Step.SUMMARY) {
      const runReport = async () => {
        ...
        try {
          ...
          setError("שגיאה בייצור הדוח הסופי. " + (err.message || "נסה שנית."));
        } finally {
          setLoadingAi(false);
        }
      };
          ...
          <div className="flex flex-col items-center space-y-10">
            ...
            <div className="space-y-4">
              ...
              <p className="text-xl md:text-2xl text-slate-400 font-medium max-w-2xl mx-auto leading-relaxed">
                מנהלים פסיכולוגיה. מעצבים עתיד. <br/> הפלטפורמה האסטרטגית לבניית תוכניות עבודה חכמות.
              </p>
            </div>
            ...
            <p className="text-slate-500 text-sm font-medium">כלי מקצועי למנהלי שפ"ח בישראל</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-950/90 backdrop-blur-md animate-in fade-in duration-300">
          <div className="glass-card max-w-2xl w-full p-10 md:p-16 rounded-[3rem] border-amber-500/20 shadow-2xl text-center space-y-8">
            ...
            <div className="space-y-4">
              ...
              <p className="text-slate-400 italic">זמן לחשיבה עמוקה:</p>
            </div>
      ...
      <main className="flex-1 container mx-auto p-4 md:p-8 flex flex-col lg:flex-row gap-8">
        <div className="flex-1 space-y-6">
          ...
          <div className="flex justify-between items-center px-4 no-print">
            <button onClick={prevStep} className={`flex items-center gap-2 px-8 py-4 text-slate-500 hover:text-white transition-all font-bold ${currentStep === Step.CONTEXT ? 'opacity-0 pointer-events-none' : ''}`}><ChevronRight className="w-5 h-5" /> חזרה</button>
            <button onClick={nextStep} disabled={currentStep === Step.SUMMARY} className={`group flex items-center gap-3 px-14 py-4 bg-amber-500 hover:bg-amber-400 text-slate-950 font-black rounded-2xl transition-all shadow-xl active:scale-95 ${currentStep === Step.SUMMARY ? 'opacity-0 pointer-events-none' : ''}`}>המשך <ChevronLeft className="group-hover:-translate-x-1 transition-transform" /></button>
          </div>
        </div>
        ...
        <aside className="w-full lg:w-80 shrink-0 space-y-6 no-print">
          <div className="glass-card p-8 rounded-[2.5rem] border-amber-500/10 sticky top-32">
            ...
            {error && (
              <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-2xl text-red-400 text-xs flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  <AlertCircle className="w-4 h-4 shrink-0" />
                  <span className="font-bold">שגיאה:</span>
                </div>
                <p className="leading-relaxed">{error}</p>
                <button onClick={fetchSuggestions} className="flex items-center gap-2 text-[10px] underline hover:text-red-300 transition-colors">
                  <RefreshCcw className="w-3 h-3" /> נסה שנית עכשיו
                </button>
              </div>
            )}

            <button onClick={fetchSuggestions} disabled={loadingAi} className="w-full py-4 bg-slate-900 border border-slate-800 rounded-xl mb-6 font-bold flex items-center justify-center gap-3 hover:bg-slate-800 transition-colors disabled:opacity-50 text-slate-200 shadow-inner">
            ...
            <div className="bg-slate-950/40 p-5 rounded-2xl border border-slate-800 min-h-[350px] text-[13px] leading-relaxed text-slate-300 overflow-y-auto custom-scrollbar">
              {aiSuggestions ? <div className="whitespace-pre-wrap animate-in fade-in duration-500">{aiSuggestions}</div> : <div className="flex flex-col items-center justify-center h-full opacity-20 text-center py-10"><BrainCircuit className="w-16 h-16 mb-4" /><p>ה-AI מחכה לנתונים שלך כדי להציע רעיונות לשיפור</p></div>}
            </div>
          </div>
        </aside>
      </main>
...
function renderStepContent(step: Step, data: WorkPlanData, setData: any, finalAiReport: string, activeTab: string, setActiveTab: any) {
  const inputClass = "w-full bg-slate-950/40 border border-slate-800 rounded-3xl p-8 focus:ring-2 focus:ring-amber-500 outline-none text-xl leading-relaxed text-slate-200 shadow-inner transition-all";
  ...
  switch (step) {
    case Step.CONTEXT:
      return <textarea className={inputClass + " h-80"} placeholder="תאר את הרקע, הסביבה המקצועית והתחושות הנוכחיות של היחידה לקראת השנה הבאה..." value={data.selfContext} onChange={(e) => setData({...data, selfContext: e.target.value})} />;
    case Step.SWOT:
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {[
            ...
            <div key={cat.key} className={`p-8 rounded-[2.5rem] border border-slate-800 ${cat.bg} hover:border-slate-700 transition-colors`}>
              ...
              <ul className="space-y-2">
                {(data.swot[cat.key as keyof SwotData] as string[]).map((item, i) => (
                  <li key={i} className="flex justify-between items-center text-sm bg-slate-950/80 p-4 rounded-xl border border-slate-800 shadow-lg group">
                    <span className="text-slate-200">{item}</span>
                    <button onClick={() => { const nl = [...(data.swot[cat.key as keyof SwotData])]; nl.splice(i,1); setData({...data, swot:{...data.swot, [cat.key]:nl}}); }} className="text-slate-700 hover:text-red-400 transition-colors opacity-0 group-hover:opacity-100"><Trash2 className="w-4 h-4" /></button>
                  </li>
                ))}
              </ul>
            </div>
      ...
      return (
        <div className="space-y-8">
          <div className="flex gap-4">
            <input id="goal-in" className="flex-1 bg-slate-950 border border-slate-800 rounded-2xl px-8 py-5 text-xl text-slate-200 shadow-inner" placeholder="הגדר מטרת על אסטרטגית..." />
            <button onClick={() => { const el = document.getElementById('goal-in') as HTMLInputElement; if(el.value) { addItem('highLevelGoals', el.value); el.value=''; } }} className="bg-amber-500 text-slate-950 px-10 rounded-2xl font-black shadow-lg hover:bg-amber-400 transition-all active:scale-95">הוסף מטרה</button>
          </div>
          <div className="space-y-4">
            {data.highLevelGoals.map((g, i) => (
              <div key={i} className="flex justify-between items-center bg-slate-900/40 p-8 rounded-[2rem] border border-slate-800 hover:border-amber-500/20 transition-all">
        ...
        <div className="space-y-12">
          {data.highLevelGoals.map((goal, gIdx) => (
            <div key={gIdx} className="p-10 border border-slate-800 rounded-[3rem] bg-slate-950/20 relative shadow-2xl">
              <div className="absolute -top-4 right-10 bg-slate-900 border border-slate-800 px-6 py-2 rounded-full text-[10px] text-amber-500 font-black uppercase tracking-widest">מטרה: {goal}</div>
              <div className="flex gap-4 mb-8">
                ...
                <button onClick={() => { const el = document.getElementById(`obj-in-${gIdx}`) as HTMLInputElement; if(el.value) { addItem('objectives', { id: Date.now().toString(), goalIndex: gIdx, text: el.value }); el.value=''; } }} className="bg-slate-800 text-white px-8 rounded-xl font-bold border border-slate-700 hover:bg-slate-700">הוסף יעד</button>
              </div>
          ...
          {data.objectives.map(obj => (
            <div key={obj.id} className="p-10 border border-slate-800 rounded-[3rem] bg-slate-950/20 shadow-xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="space-y-1"><label className="text-[10px] text-slate-500 mr-2 font-bold">משימה</label><input id={`t-d-${obj.id}`} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-sm text-slate-200" placeholder="תיאור המשימה..." /></div>
                <div className="space-y-1"><label className="text-[10px] text-slate-500 mr-2 font-bold">אחראי</label><input id={`t-r-${obj.id}`} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-sm text-slate-200" placeholder="מי האחראי?" /></div>
                <div className="space-y-1"><label className="text-[10px] text-slate-500 mr-2 font-bold">לו''ז</label><input id={`t-t-${obj.id}`} className="w-full bg-slate-900 border border-slate-800 rounded-xl p-4 text-sm text-slate-200" placeholder="לו''ז..." /></div>
                ...
                {data.tasks.filter(t => t.objectiveId === obj.id).map(task => (
                  <div key={task.id} className="flex justify-between items-center bg-slate-950/60 p-5 rounded-xl border border-slate-800 text-sm shadow-md group">
                    <div className="flex gap-8 items-center flex-1">
                      ...
                      <span className="flex items-center gap-2 text-slate-400 bg-slate-900 px-3 py-1 rounded-lg"><User className="w-4 h-4 text-amber-500/50"/> {task.responsibility || '-'}</span>
                      <span className="flex items-center gap-2 text-slate-500 italic bg-slate-900 px-3 py-1 rounded-lg"><Calendar className="w-4 h-4 text-amber-500/50"/> {task.timeline || '-'}</span>
                    </div>
                    <button onClick={() => removeItem('tasks', task.id)} className="text-slate-700 hover:text-red-400 transition-all"><Trash2 className="w-4 h-4"/></button>
                  </div>
                ))}
              </div>
            </div>
    ...
    case Step.CONSTRAINTS:
      return <textarea className={inputClass + " h-80 border-red-500/10 shadow-[inset_0_0_50px_rgba(239,68,68,0.02)]"} placeholder="פרט כאן אילוצים תקציביים, חסמי כוח אדם, או סיכונים חיצוניים..." value={data.constraints} onChange={(e) => setData({...data, constraints: e.target.value})} />;
    case Step.SUMMARY:
      return (
        <div className="space-y-12 animate-in fade-in duration-1000">
          <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-slate-800 pb-10">
            <div><h2 className="text-4xl font-black text-white tracking-tight">תוכנית עבודה שנתית</h2><p className="text-slate-500 italic mt-2 text-lg">שירות פסיכולוגי חינוכי - גרסה מוגמרת</p></div>
            <div className="flex p-1.5 bg-slate-950 rounded-2xl border border-slate-800 no-print">
              <button onClick={() => setActiveTab('original')} className={`px-8 py-3 rounded-xl text-sm font-black transition-all ${activeTab==='original'?'bg-slate-800 text-white shadow-xl':'text-slate-600 hover:text-slate-400'}`}>טיוטה גולמית</button>
              <button onClick={() => setActiveTab('ai')} className={`px-8 py-3 rounded-xl text-sm font-black flex items-center gap-2 transition-all ${activeTab==='ai'?'bg-amber-500 text-slate-950 shadow-xl':'text-slate-600 hover:text-amber-500'}`}><Sparkles className="w-4 h-4"/> גרסת Master (AI)</button>
            </div>
          </div>
          <div className="glass-card rounded-[3.5rem] p-10 md:p-20 shadow-2xl relative bg-slate-950/40 border border-slate-800 min-h-[600px] overflow-x-auto">
            {activeTab === 'original' ? (
              <div className="space-y-16">
                <section className="text-center"><h4 className="text-amber-500 font-black text-[10px] uppercase tracking-[0.4em] mb-8">חזון השירות</h4><p className="text-5xl font-serif italic text-white leading-tight">"{data.vision || 'לא הוגדר חזון'}"</p></section>
                <section>
                   <div className="overflow-x-auto rounded-[3rem] border border-slate-800 shadow-2xl overflow-hidden">
                     <table className="w-full text-sm">
                       <thead className="bg-slate-900 text-slate-400 text-xs font-black uppercase tracking-widest"><tr><th className="p-8 text-right">יעד SMART</th><th className="p-8 text-right">משימה</th><th className="p-8 text-right">אחריות</th><th className="p-8 text-right">לו''ז</th></tr></thead>
                       <tbody>
                        {data.objectives.map(obj => {
                          const objTasks = data.tasks.filter(t => t.objectiveId === obj.id);
                          return objTasks.length === 0 ? <tr key={obj.id} className="border-t border-slate-900"><td className="p-8 font-black text-amber-500/70">{obj.text}</td><td colSpan={3} className="p-8 italic opacity-20 text-center">אין משימות ביצוע</td></tr> :
                          objTasks.map((t, idx) => <tr key={t.id} className="border-t border-slate-900/50 hover:bg-slate-900/10 transition-colors">
                            ...
                            <td className="p-8 text-slate-400"><span className="bg-slate-900/50 px-3 py-1 rounded-lg border border-slate-800">{t.responsibility}</span></td>
              ...
              <div className="animate-in fade-in duration-1000">
                {finalAiReport ? <div className="prose prose-invert max-w-none prose-amber p-4" dangerouslySetInnerHTML={{ __html: formatMarkdown(finalAiReport) }} /> : (
                  <div className="flex flex-col items-center justify-center h-[500px] space-y-10">
                    <div className="relative"><div className="animate-ping absolute inset-0 bg-amber-500 opacity-20 rounded-full" /><Sparkles className="relative w-20 h-20 text-amber-500 animate-pulse" /></div>
                    <p className="text-2xl font-black text-white tracking-widest">מייצר אינטגרציה אסטרטגית סופית...</p>
                    <p className="text-slate-500 italic max-w-sm text-center">ה-AI משלב את ניתוח ה-SWOT, האילוצים והמטרות לכדי תוכנית עבודה מהודקת.</p>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="flex justify-center gap-6 pt-10 no-print">
            <button onClick={() => window.print()} className="flex items-center gap-5 bg-white text-slate-950 hover:bg-amber-500 transition-all px-16 py-6 rounded-[2.5rem] font-black shadow-2xl active:scale-95"><Save className="w-5 h-5" /> הדפס תוכנית</button>
          </div>
        </div>
      );
    default: return null;
...
function formatMarkdown(text: string) {
  let html = text
    .replace(/^### (.*$)/gim, '<h3 class="text-2xl font-black text-amber-500 mt-12 mb-6 border-r-4 border-amber-500 pr-4">$1</h3>')
    ...
    .replace(/^\* (.*$)/gim, '<li class="mr-10 mb-4 text-slate-300 text-lg list-disc">$1</li>')
    .replace(/\*\*(.*)\*\*/gim, '<strong class="text-white font-black">$1</strong>')
    .replace(/\n/g, '<br />');

  if (text.includes('|')) {
    const lines = text.split('\n');
    let tableHtml = '<div class="overflow-x-auto my-12 rounded-[2rem] border border-slate-800 shadow-2xl overflow-hidden"><table class="w-full border-collapse bg-slate-950/40 text-xs">';
    lines.forEach(line => {
      if (line.includes('|')) {
        const cells = line.split('|').filter(c => c.trim().length > 0 || line.includes('---'));
        if (line.includes('---')) return;
        const isHeader = line.toLowerCase().includes('מטרה') || line.toLowerCase().includes('יעד') || line.toLowerCase().includes('משימה') || line.toLowerCase().includes('אחראי');
    ...
    // Attempt to swap the markdown table string with the HTML table
    const firstPipe = html.indexOf('|');
    if (firstPipe !== -1) {
       // This is a simplified replacement for display purposes
       return html.split('|')[0] + tableHtml + html.split('|').pop();
    }
  }
  return html;
}
